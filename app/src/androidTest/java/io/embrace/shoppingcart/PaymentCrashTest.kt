package io.embrace.shoppingcart

import androidx.compose.ui.test.hasTestTag
import androidx.compose.ui.test.junit4.AndroidComposeTestRule
import androidx.compose.ui.test.junit4.createAndroidComposeRule
import androidx.compose.ui.test.performClick
import androidx.test.espresso.IdlingPolicies
import androidx.test.ext.junit.runners.AndroidJUnit4
import android.content.Intent
import androidx.test.platform.app.InstrumentationRegistry
import androidx.test.uiautomator.UiDevice
import androidx.test.uiautomator.By
import androidx.test.uiautomator.Until
import io.embrace.shoppingcart.presentation.testutil.UiTestOverrides
import io.embrace.shoppingcart.ui.payment.PaymentMethodsActivity
import java.util.concurrent.TimeUnit
import org.junit.AfterClass
import org.junit.Before
import org.junit.BeforeClass
import org.junit.Rule
import org.junit.Test
import org.junit.runner.RunWith

// Mirrors CrashTest, but for the payment flow. Launches PaymentMethodsActivity
// directly (the crash button lives on PaymentMethodsScreen) and taps the demo
// crash button. UiTestOverrides.forcePaymentCrash forces the normally-~30%
// crash to fire 100% of the time so this test is deterministic.
@RunWith(AndroidJUnit4::class)
class PaymentCrashTest {

    @get:Rule
    val composeRule: AndroidComposeTestRule<*, *> =
        createAndroidComposeRule<PaymentMethodsActivity>()

    private val device: UiDevice by lazy {
        UiDevice.getInstance(InstrumentationRegistry.getInstrumentation())
    }

    companion object Companion {
        @JvmStatic
        @BeforeClass
        fun forceCrash() {
            UiTestOverrides.forcePaymentCrash = true
        }

        @JvmStatic
        @AfterClass
        fun resetCrash() {
            UiTestOverrides.forcePaymentCrash = false
        }
    }

    // Slow CI emulators (notably API 31 x86_64) can skip hundreds of frames during
    // cold start, leaving the Compose-Espresso idling resource non-idle past the
    // default 26s timeout. Bump to 60s to absorb that.
    @Before
    fun raiseIdlingTimeouts() {
        IdlingPolicies.setMasterPolicyTimeout(60, TimeUnit.SECONDS)
        IdlingPolicies.setIdlingResourceTimeout(60, TimeUnit.SECONDS)
    }

    @Test
    fun payment_crash() {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        val packageName = context.packageName

        // Wait for the crash button and trigger the crash
        composeRule.waitUntil(conditionDescription = "payment_crash_button", 30_000) {
            composeRule.onAllNodes(hasTestTag("payment_crash_button"), useUnmergedTree = true)
                .fetchSemanticsNodes().isNotEmpty()
        }
        composeRule.onAllNodes(hasTestTag("payment_crash_button"), useUnmergedTree = true)[0]
            .performClick()

        // Wait for async crash to occur (100ms delay in Handler + crash processing time)
        Thread.sleep(2_000)

        // Use UiDevice to dismiss any crash dialogs (more reliable than Espresso after crash)
        repeat(3) {
            try {
                device.pressBack()
                Thread.sleep(300)
            } catch (_: Exception) {
                // Ignore errors - crash dialogs may not exist
            }
        }

        // Wait for crash to be fully processed
        Thread.sleep(1_000)

        // Verify app is not running after crash
        val appStillRunning = device.wait(Until.hasObject(By.pkg(packageName)), 500)
        if (appStillRunning) {
            println("Warning: App still running after crash, forcing close")
            device.pressBack()
            Thread.sleep(500)
        }

        // Relaunch the app
        val intent = context.packageManager.getLaunchIntentForPackage(packageName)?.apply {
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK)
        }

        if (intent == null) {
            throw AssertionError("Could not get launch intent for package: $packageName")
        }

        context.startActivity(intent)

        // Wait for app to appear (using UiDevice, more reliable than composeRule)
        val appRestarted = device.wait(Until.hasObject(By.pkg(packageName)), 15_000)
        if (!appRestarted) {
            throw AssertionError("Failed to restart app after crash - app did not appear")
        }

        // Give app additional time to fully initialize and settle
        Thread.sleep(4_000)

        // Test passes if we got here - app crashed and successfully restarted
        println("SUCCESS: Payment screen crashed and app restarted successfully")
        assert(true)
    }
}
